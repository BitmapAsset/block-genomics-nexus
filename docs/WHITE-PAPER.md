...
Beyond binary identity verification, Block Genomics computes a multi-factor **Trust Score** for each genome:
...
Weights (w₁–w₅) are configurable per application. A financial protocol might weight Security and Ownership heavily; a social platform might prioritize History.
